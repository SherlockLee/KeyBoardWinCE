...
//#define MB_TCP_POOL_TIMEOUT 10  /* pool timeout for event waiting. */
...
#define MB_TCP_DEFAULT_PORT 502		/* TCP listening port. */
#define MB_TCP_POOL_TIMEOUT 5		/* pool timeout for event waiting. */
...
prvMBTCPGetFrame(  )
{
    ...
    iRes = recv( xClientSocket, ((CHAR *)&aucTCPBuf[usTCPBufPos]), usTCPFrameBytesLeft, 0 );
    switch ( iRes )
    {
    case SOCKET_ERROR:
...
xMBTCPPortSendResponse( const UCHAR * pucMBTCPFrame, USHORT usTCPLength )
{
    ...
    do
    {
        res = send( xClientSocket, ((CHAR *)&pucMBTCPFrame[iBytesSent]), usTCPLength - iBytesSent, 0 );
        switch ( res )
        {
        case SOCKET_ERROR:
...
prvvMBPortReleaseClient(  )
{
     ...
     */
    ( void )recv( xClientSocket, ((CHAR *)&aucTCPBuf[0]), MB_TCP_BUF_SIZE, 0 );
